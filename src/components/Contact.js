    ...
    return (
        <div className="col-12 mainelement contact">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <input type="text" placeholder="Ole Nordmann" name="name" ref={register({required:true})}/>
                {errors.name && <ErrorMessage content={errors.name.message}/>}
                <label><p>E-post</p></label>
                <input type="text" placeholder="ole.nordmann@gmail.com" name="email" ref={register({required:true})}/>
                {errors.email && <ErrorMessage content={errors.email.message}/>}
                <label><p>Melding</p></label>
                <textarea placeholder="Din melding her" name="message" ref={register({required:true})}></textarea>
